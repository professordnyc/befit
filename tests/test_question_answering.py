...
async def test_context_interpreter_fallback_preserves_user_question():
    ...
    assert (
        intent.get("user_question") == RAW_QUESTION
...
async def test_pipeline_answers_question_directly():
    ...
    question_terms = {"raw", "cook", "cooked", "cooking"}
    matched = question_terms & set(action_text.split())
    assert matched, f"Expected the first action to address raw/cook. Got: {action_text!r}"
            ...
            "actions": [
                {
                    ...
                    "description": (
                        "Toss some spinach into a smoothie or salad " "for an easy nutrient boost."
                    ),
                },
                {
                    "title": "Include more protein",
                    "description": (
                        "Chicken breast is a great lean protein -- " "try it in a stir-fry tonight."
                    ),
                },
            ],
