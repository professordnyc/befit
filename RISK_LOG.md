...
- [ ] Always-on mic mode: a single `SpeechRecognition` instance is shared between query input and TTS command listening; mode is controlled by the `ttsListening` flag. If the query-mode session restarts during a pipeline fetch (e.g. always-on), it may consume the mic before the cmd listener arms — mitigated by keeping `ttsAudio` non-null during the fetch and gating the always-on restart on `!ttsFetching`.
